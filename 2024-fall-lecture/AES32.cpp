...
void AES32_EncRound(u32 st[4], u32 rk[4]) {
	...
	tmp[0] = Te0[st[0] >> 24] ^ Te1[(st[1] >> 16) & 0xff] ^ Te2[(st[2] >> 8) & 0xff] ^ Te3[st[3] & 0xff] ^ rk[0];
	tmp[1] = Te0[st[1] >> 24] ^ Te1[(st[2] >> 16) & 0xff] ^ Te2[(st[3] >> 8) & 0xff] ^ Te3[st[0] & 0xff] ^ rk[1];
	tmp[2] = Te0[st[2] >> 24] ^ Te1[(st[3] >> 16) & 0xff] ^ Te2[(st[0] >> 8) & 0xff] ^ Te3[st[1] & 0xff] ^ rk[2];
	tmp[3] = Te0[st[3] >> 24] ^ Te1[(st[0] >> 16) & 0xff] ^ Te2[(st[1] >> 8) & 0xff] ^ Te3[st[2] & 0xff] ^ rk[3];
	for (int k = 0; k < 4; k++) {
		st[k] = tmp[k];
	}
}
...
void AES32_EqDecRound(u32 st[4], u32 rk[4]) {
	...
	tmp[3] = Td0[st[3] >> 24] ^ Td1[(st[2] >> 16) & 0xff] ^ Td2[(st[1] >> 8) & 0xff] ^ Td3[st[0] & 0xff] ^ rk[3];
	for (int k = 0; k < 4; k++) {
		st[k] = tmp[k];
	}
}
...
void AES32_EqDecrypt(byte ct[16], u32 rk[11][4], byte pt[16]) {
	...
	tmp[1] = (Td4[st[1] >> 24] & 0xff000000) ^ (Td4[(st[0] >> 16) & 0xff] & 0x00ff0000)
		^ (Td4[(st[3] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[st[2] & 0xff] & 0x000000ff) ^ rk[0][1];
	tmp[2] = (Td4[st[2] >> 24] & 0xff000000) ^ (Td4[(st[1] >> 16) & 0xff] & 0x00ff0000)
		^ (Td4[(st[0] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[st[3] & 0xff] & 0x000000ff) ^ rk[0][2];
	tmp[3] = (Td4[st[3] >> 24] & 0xff000000) ^ (Td4[(st[2] >> 16) & 0xff] & 0x00ff0000)
		^ (Td4[(st[1] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[st[0] & 0xff] & 0x000000ff) ^ rk[0][3];
