...
ax.plot(function1_smooth, color='dodgerblue', label='8-bit AES', alpha=0.8, linewidth=2)
ax.plot(function2_smooth, color='crimson', label='32-bit AES', alpha=0.8, linewidth=2)

# Add horizontal lines to indicate the average values
ax.hlines(average_function1, xmin=0, xmax=len(function1_smooth), colors='dodgerblue', linestyles='dotted', label=f'Avg 8-bit AES: {average_function1:.2f} µs')
ax.hlines(average_function2, xmin=0, xmax=len(function2_smooth), colors='crimson', linestyles='dotted', label=f'Avg 32-bit AES: {average_function2:.2f} µs')
