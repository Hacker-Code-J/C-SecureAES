def sub_word(word, s_box):
    """ Substitute bytes in a word using the S-box. """
...
def key_expansion(key, s_box):
    """ Generate key schedule for AES-128 from the given key. """
    Nk = 4  # Number of 4 byte words in the key
    ...
    # Initial copy of key
    for i in range(Nk):
        w[i] = int.from_bytes(key[4*i:4*(i+1)], byteorder='big')

    # Key expansion
    for i in range(Nk, Nb * (Nr + 1)):
        temp = w[i - 1]
        if i % Nk == 0:
            temp = int.from_bytes(sub_word(rot_word(temp.to_bytes(4, byteorder='big')), s_box), byteorder='big') ^ Rcon[i//Nk - 1]
        w[i] = w[i - Nk] ^ temp
